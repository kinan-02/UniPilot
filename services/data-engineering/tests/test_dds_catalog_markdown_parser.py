...
def test_parse_curated_catalog_draft_from_fixture(tmp_path: Path) -> None:
    ...
    assert faculty_pool.courseReferences == []
    assert faculty_pool.ruleExpression.get("allowedPrefixes")
...
def test_credit_buckets_on_fixture() -> None:
    document, _report = parse_curated_catalog_draft(str(FIXTURE_PATH))
    ds = next(p for p in document.programs if p.programCode == "009216-1-000")
    buckets = {
        g.groupId.split(":")[-1]: g.minCredits
        for g in ds.requirementGroups
        if g.ruleExpression.get("type") == "credit_bucket"
    }
    assert buckets["core-mandatory"] == 108.0
    assert buckets["elective-ds"] == 24.5
    assert buckets["elective-faculty"] == 10.5
    assert buckets["elective-general"] == 12.0
