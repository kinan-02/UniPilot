...
from app.agent_core.loop.runner import _report_progress, run_agent_loop
...
def _progress_ctx(sink: list[str]) -> SimpleNamespace:
    return SimpleNamespace(on_progress=sink.append, last_progress=None)


def test_consecutive_duplicate_phrases_are_suppressed():
    """The same sentence twice reads as a stalled request -- the opposite of what
    progress exists to convey."""
    sink: list[str] = []
    ctx = _progress_ctx(sink)

    _report_progress(ctx, [{"tool": "select"}])
    _report_progress(ctx, [{"tool": "select"}])
    _report_progress(ctx, [{"tool": "compute"}])
    _report_progress(ctx, [{"tool": "select"}])
...
def test_a_raising_progress_callback_does_not_cost_the_turn():
    ...
    _report_progress(SimpleNamespace(on_progress=_explode, last_progress=None), [{"tool": "get_entity"}])
