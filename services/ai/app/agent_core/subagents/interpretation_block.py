"""`InterpretationReasoningBlock` -- a purpose-built, alternate
dispatch target for the `interpretation` role
(docs/agent/agent_plans/INTERPRETATION_REASONING_BLOCK_PLAN.md).

Extends `BaseReasoningBlock` with the same bounded per-round loop shape
`RetrievalReasoningBlock` established, using the shared
`subagents.tool_round.execute_tool_round` helper. Two deliberate
differences from retrieval:

1. `_MIN_ROUNDS = 2` -- unlike retrieval, there is no "nothing to look up"
   case for this role; interpreting always requires at least one
   `interpret_text` (or resolving) tool call. A `status: "ready"` on round 1,
   before any tool has run, is never honored.
2. `source_ref` is a REQUIRED output field (retrieval's is optional) --
   structurally enforces "must cite the exact page/section read." If no
   source was ever successfully interpreted, finalize cannot produce a
   schema-valid result and correctly fails closed, rather than needing
   separate logic to detect a missing citation.
"""
...
_INTERPRETATION_ROUND_V1 = "interpretation_round_v1"
_INTERPRETATION_OUTPUT_SCHEMA_NAME = "interpretation_agent_output_v1"

_INTERPRETATION_OUTPUT_SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
        "certainty_basis": {
            "type": "string",
...
def _interpretation_round_contract() -> PromptContract:
    return PromptContract(
        name=_INTERPRETATION_ROUND_V1,
        version="1.0.0",
        role_prompt=(
        ...
        instructions=[
            ...
            "entry to 'warnings' that this specific part of the claim is unverifiable rather "
        ...
        output_schema_name=_INTERPRETATION_OUTPUT_SCHEMA_NAME,
...
def _build_prompt_registry() -> PromptRegistry:
    ...
    registry.register(_interpretation_round_contract())
    return registry
...
class _InterpretationBlockInput(BaseReasoningBlockInput):
    tool_grant: list[str] = Field(default_factory=list)


class _InterpretationBlockOutput(BaseReasoningBlockOutput):
    tool_audit_trail: list[ToolInvocationRecord] = Field(default_factory=list)
    rounds_used: int = 0


class InterpretationReasoningBlock(BaseReasoningBlock):
    def __init__(
        self,
        *,
    ...
    async def _run_internal(
    ...
    ) -> _InterpretationBlockOutput:
        contract = self._resolve_prompt_contract(block_input.prompt_contract_name or _INTERPRETATION_ROUND_V1)
        ...
        round_schema = {
            ...
            "properties": {
                ...
                "result": _INTERPRETATION_OUTPUT_SCHEMA,
            },
            "required": ["status"],
        }
        ...
        while round_num < _MAX_ROUNDS:
            ...
            call_result = await self._invoke_llm(
                ...
                phase=f"interpretation_round_{round_num}",
            ...
            accumulated_warnings.extend(parsed.get("warnings") or [])
            ...
            if candidate_result is None:
                return self._interpretation_failed_output(
            ...
            candidate = self._normalize_result(candidate_result, output_schema=_INTERPRETATION_OUTPUT_SCHEMA)
            validation = self._validate_schema(candidate, _INTERPRETATION_OUTPUT_SCHEMA)
            ...
            return _InterpretationBlockOutput(
                status="completed",
                schema_valid=True,
                result=candidate,
    ...
    def _interpretation_failed_output(
        self,
        *,
        reason: str,
        tool_audit_trail: list[ToolInvocationRecord] | None = None,
        rounds_used: int = 0,
        ...
        return _InterpretationBlockOutput(
            ...
            result=None,
            confidence=0.0,
            warnings=[*(warnings or []), f"interpretation_failed: {reason}"],
            tool_audit_trail=tool_audit_trail or [],
            rounds_used=rounds_used,
        )

    def _failed_output(self, block_input: BaseReasoningBlockInput, *, reason: str) -> _InterpretationBlockOutput:
        return self._interpretation_failed_output(reason=f"reasoning_block_failed: {reason}")


async def run_interpretation_subagent(
    *,
    ...
    if output.result is not None:
        basis = output.result.get("certainty_basis", "llm_interpretation")
...
__all__ = [
    "_INTERPRETATION_ROUND_V1",
    "_INTERPRETATION_OUTPUT_SCHEMA_NAME",
    "InterpretationReasoningBlock",
