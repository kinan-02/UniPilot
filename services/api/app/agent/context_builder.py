"""Centralized context assembly for agent workflows (spec §12, Phase 7 agentic RAG)."""
...
from app.retrieval.profiles import RetrievalProfile, get_profile, primary_profile_for_intent, select_profiles_for_intent
...
async def _retrieve_wiki_multi_step(
    *,
    ...
    for sub_query in queries:
        snippets, records, wiki_meta = await retrieve_wiki_context_with_profile(
            query=sub_query,
...
async def build_agent_context_pack(
    ...
    *,
    ...
    for attempt in range(max_attempts):
        ...
        if not structured_loaded or attempt == 0:
        ...
        wiki_steps = [step for step in retrieval_plan if step.get("source") == "obsidian_wiki"]
        ...
        if cfg.is_agent_llm_validation_enabled() and validation.status != "valid":
            llm_validation = await asyncio.to_thread(
                validate_retrieval_with_llm,
                pack,
                user_message=user_message,
                settings=cfg,
            )
