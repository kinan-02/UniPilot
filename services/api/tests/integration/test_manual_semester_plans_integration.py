...
@pytest.mark.asyncio
async def test_create_manual_semester_plan_without_degree(auth_client, mongo_database):
    fixtures = await seed_graduation_progress_fixtures(mongo_database)
    token = await register_access_token(auth_client, "manual-plan-no-degree@example.com")
    ...
    plan = response.json()["data"]["semesterPlan"]
    assert plan["plannerType"] == "manual"
    assert len(plan["semesters"][0]["plannedCourses"]) == 1
