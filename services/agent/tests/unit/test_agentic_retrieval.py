...
def test_decompose_skips_clause_split_for_wiki_entity_intents():
    message = (
        "I'm a DDS student interested in the Inter-Faculty Robotics Minor. "
        "What are the admission requirements, and how many credits do I need to complete it?"
    )
    queries = decompose_retrieval_query(
        user_message=message,
        intent="program_minor_lookup",
        entities={"programSlug": "minor-robotics"},
        base_wiki_query=message,
    )
    assert len(queries) == 1
    assert queries[0].source == "base"
