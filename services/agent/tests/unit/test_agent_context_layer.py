...
def test_retrieval_plan_orders_structured_sources_first():
    ...
    # `academic_graph` replaced the legacy `obsidian_wiki` source when the
    # wiki-graph retriever took over from the hybrid RAG retriever.
    assert any(step["source"] == "academic_graph" for step in steps)
