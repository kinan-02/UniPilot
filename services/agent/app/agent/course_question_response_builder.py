...
from app.services.prerequisite_validation_service import validation_to_prerequisite_result
...
def build_course_question_blocks(
    *,
    ...
    if analysis.prerequisite_validation is not None:
        prereq = validation_to_prerequisite_result(analysis.prerequisite_validation)
...
def build_course_question_text(*, analysis: CourseQuestionAnalysis) -> str:
    if analysis.use_catalog_answer and analysis.catalog_answer:
        return analysis.catalog_answer.strip()
    if analysis.use_eligibility_validation and analysis.eligibility_answer:
        parts = [analysis.eligibility_answer.strip()]
...
def _prerequisite_status_block(
    *,
    ...
    if not prereq:
        status = "unknown"
    elif prereq.get("eligibilityStatus") == "unknown":
        status = "unknown"
    ...
    return StructuredBlock(
        ...
        data={
            ...
            "courseName": course.get("title") or course.get("titleHebrew"),
            "status": status,
            "satisfiedPrerequisites": prereq.get("satisfiedPrerequisiteIds") or [],
            "missingPrerequisites": prereq.get("missingPrerequisites") or [],
            "reason": prereq.get("reason"),
            "notes": prereq.get("reason"),
            "eligibilityStatus": prereq.get("eligibilityStatus"),
            "requiredPrerequisiteCodes": prereq.get("requiredPrerequisiteCodes") or [],
            "completedCourseCount": len(prereq.get("completedCourseCodes") or []),
        },
    )
