...
class AcademicGraphEngine:
    ...
    def search_wiki(
        ...
        # `rerank_chunks` scores every chunk it's given (BM25 always, plus an
        # embedding lookup when configured) — reranking the entire
        # multi-thousand-chunk wiki corpus on every call would trade the old
        # cheap-but-low-quality keyword count for a real latency regression,
        # not just a one-time cost (measured: reranking ~4.7k loosely-matched
        # candidates took ~9s). Score a cheap token-match count first (same
        # cost profile as the previous implementation's own substring check —
        # a fraction of a second even over the full corpus), then only rerank
        # the top `_WIKI_SEARCH_CANDIDATE_POOL_CAP` of those with the real
        ...
        candidates = [chunk for chunk, _count in match_counts[:_WIKI_SEARCH_CANDIDATE_POOL_CAP]]
