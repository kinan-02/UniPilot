...
def semester_filename_for_plan_code(plan_semester_code: str | None) -> str | None:
    """Map profile plan code (e.g. 2025-1) to semester offering JSON filename."""
    raw = str(plan_semester_code or "").strip()
    parts = raw.split("-", 1)
    if len(parts) != 2 or not parts[0].isdigit() or parts[1] not in {"1", "2", "3"}:
        return None
    file_academic_year = int(parts[0])
    offering_code = int(parts[1]) + 199
    return f"courses_{file_academic_year}_{offering_code}.json"
