...
class AcademicGraphEngine:
    ...
    def evaluate_eligibility(
        ...
        completed = _expand_completed_course_keys(completed_courses)
...
def _expand_completed_course_keys(completed_courses: list[str] | set[str] | None) -> set[str]:
    expanded: set[str] = set()
    for raw in completed_courses or []:
        text = str(raw).strip()
        if not text:
            continue
        expanded.add(text)
...
def _course_is_completed(course_id: str, completed: set[str]) -> bool:
    if course_id in completed:
        return True
    return _normalize_course_key(course_id) in completed


def _missing_from_ast(ast: dict[str, Any], completed: set[str]) -> list[str]:
    node_type = ast.get("type")
    if node_type == "COURSE":
        return [] if _course_is_completed(ast["id"], completed) else [ast["id"]]
    if node_type == "AND":
        missing: list[str] = []
        for operand in ast.get("operands", []):
